...
concrete UDCatEng of UDCat = BareRGEng **
  open SyntaxEng, (S=SyntaxEng), Prelude, (P=ParadigmsEng), (E=ExtraEng), ResEng, ExtendEng, (N=NounEng), SymbolicEng in {
...
--    AuxType = Be | Have | Will | RealAux ;
  ...
  oper
    LinAdvmod : Type = {adv : S.Adv ; isNot : Bool} ;
    LinAux : Type = {s : Str ; vv : S.VV ; auxType : AuxType} ;
    mkAux = overload {
      mkAux : Str -> S.VV -> LinAux = \str,vv -> {
  ...
  lin
    ...
    rootV_ temp pol vp = mkRoot <temp : S.Temp> <pol : S.Pol> <vp : S.VP> ;
    rootVaux_ temp pol aux vp =
      let ant : S.Ant = lin Ant {s = [] ; a = temp.a} ;
          may_have_occurred : S.VP = ParseExtendComplVV aux.vv ant pol vp ;
  ...
  oper
    ...
    defaultUDSPred : S.VP -> UDSPred = \vp -> defaultRoot ** {
...
--    linUDS = overload {
  ...
  --  } ;
    ...
    mkUDSPred = overload {
      ...
      mkUDSPred : S.VP -> UDSPred = defaultUDSPred ;
      mkUDSPred : S.Tense -> S.VP -> UDSPred = \tns,vp -> defaultUDSPred vp ** {
        fin = MkVPS (mkTemp tns simultaneousAnt) positivePol vp } ;
      mkUDSPred : S.Ant -> S.VP -> UDSPred = \ant,vp -> defaultUDSPred vp ** {
        fin = MkVPS (mkTemp presentTense ant) positivePol vp } ;
      mkUDSPred : S.Temp -> S.Pol -> S.VP -> UDSPred = \temp,pol,vp ->
      ...
      mkUDSPred : S.NP -> UDSPred = \np ->
        let vp : S.VP = mkVP np in {
        fin = MkVPS (presSimulTemp) positivePol vp ;
        pp = BareRGEng.PastPartAP vp ;
        presp = BareRGEng.PresPartAP vp ;
        inf = ExtendEng.MkVPI vp ;
    ...
    mkRoot = overload {
       mkRoot : S.AP -> LinRoot = \ap -> emptyRoot ** {vp = mkVP ap} ;
       mkRoot : S.NP -> LinRoot = \np -> emptyRoot ** {vp = mkVP np ; np = np ; isNP = True} ;
       mkRoot : S.VP -> LinRoot = \vp -> emptyRoot ** {vp = vp} ;
       mkRoot : S.VPSlash -> LinRoot = \vp -> emptyRoot ** {vp = vp ; c2 = vp.c2} ;
       mkRoot : S.Temp -> S.Pol -> S.VP -> LinRoot = \t,p,vp ->
    ...
    presSimulTemp : S.Temp = mkTemp presentTense simultaneousAnt ;

    aclRelclRS_ : S.RS -> S.Adv = \rs -> lin Adv {s = embedInCommas (rs.s ! agrP3 Sg)} ;

    -- Add an SC onto a LinRoot, e.g.
    -- (ready : LinRoot) (to_sleep : SC) -> ready to sleep
    scRoot : LinRoot -> S.SC -> LinRoot = \rt,sc -> advRoot rt <sc : S.Adv> ;

    -- Add an Adv onto a LinRoot, e.g.
    -- (critical : LinRoot) (always : Adv) -> always critical
    -- (warm : LinRoot) (by_nature : Adv) -> warm by nature
    advRoot : LinRoot -> S.Adv -> LinRoot = \rt,adv -> rt ** {
      vp = mkVP rt.vp adv ;
      np = N.AdvNP <rt.np:S.NP> <adv:S.Adv> ;
    } ;

    -- Add a direct object onto a LinRoot, e.g.
    -- (eat : LinRoot) (food : NP) -> eat food
    dObjRoot : LinRoot -> S.NP -> LinRoot = \rt,np -> rt ** {
      np = ApposNP rt.np np ;
      vp = rt.vp ** {
    ...
    emptyNP : S.NP = it_NP ** {s = \\_ => ""} ;
    emptySubj : S.Subj = that_Subj ** {s = ""} ;

    should_VV : S.VV = lin VV {
      s = table {
    ...
    ParseExtendComplVV : S.VV -> S.Ant -> S.Pol -> S.VP -> S.VP ;
