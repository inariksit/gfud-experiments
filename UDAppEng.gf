--# -path=.:morphodict:gf-wordnet

concrete UDAppEng of UDApp =
  UDCatEng, MorphoDictEng, CustomLexEng **
  open Prelude, SyntaxEng, (S=SyntaxEng), IrregEng, ExtendEng,
    SymbolicEng
...
-- Variations on root_nsubj_*
    ...
    {- The lincat of advmod is {adv : S.Adv ; isNot : Bool} (from UDCatEng.gf)
       The two constructors for advmod are: (found in UDCat.gf)
         1) advmod_    : S.Adv -> advmod
         2) not_advmod : advmod
    ...
    root_nsubj_advmod rt sub advm =
      ...
      case advm.isNot of {
        ...
        False => -- the advmod comes from advmod_ : S.Adv -> advmod
...
-- Variations on root_obl_* / root_nmod_*
    ...
    -- [publicly] available [solely] because of any data [breach]. ;
      ...
      root_obl  -- TODO is this too overfitting? "publicly available solely …"
        (mkRoot (mkVP <publicly.adv : S.AdV> (mkVP available.vp solely.adv)))
        because_of_breach ;

    -- : root -> obl -> appos -> UDS ;
  	-- mentioned in [sub-paragraph] [(a)] -- this UDpipe output has wrong attachment
		root_obl_appos root obl appos = root_obl (advRoot root <appos : S.Adv>) obl ;
...
-----------------------------------------------------------------------------
  ...
  root_nsubj_xcomp rt ns xc =
	...
	root_xcomp render unlikely =
	  let render_unlikely : S.VP = mkVP render.vp <unlikely : S.Adv> ;
 ...
 root_advmod_nsubj_cop rt am sub _cop =
  root_nsubj (mkRoot (mkVP <am.adv : S.AdV> rt.vp)) sub ;
...
-- acl, advcl
  ...
  -- : root -> advmod -> acl -> UDS ; -- heuristic: advmod is actually AdV
	root_advmod_advcl rt am acl = root_acl (mkRoot (mkVP <am.adv : S.AdV> rt.vp)) acl ;
...
-- Needed to add special funs that are not the top-level/application layer
  ...
  oper
    ...
    addRcl : LinRoot -> S.Adv -> LinRoot = \rt,rs ->
      let dummyNP : S.NP = mkNP emptyNP rs ;
          RSasAdv : S.Adv = lin Adv (mkUtt dummyNP) ;
       in advRoot rt rs ;
    ...
    addMark : Subj -> LinUDS -> LinUDS = \if,breachOccurs ->
      let if_Predet : S.Predet = lin Predet if ; -- hack: Predet and Subj have the same lincat in English RG
          breach_NP : S.NP = breachOccurs.subj ;
